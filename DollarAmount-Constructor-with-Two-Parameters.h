...
public:
    ...
    DollarAmount(int64_t dollars, int64_t cents) : amount(dollars * 100)      // multiplay dollars by 100 to chang it to cents and save 
                                                                              // it to amount(the total by cents)
    {
        amount += cents;
    }
    ...
    void addInterest(int rate, int divisor)
    {
        DollarAmount  interest{
           0, (amount * rate + divisor / 2) / divisor };                       //put 0 in the place of dollars constractor to match 
                                                                               // the calss of two constractors 
